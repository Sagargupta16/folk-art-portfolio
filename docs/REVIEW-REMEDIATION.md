...
| Scripts are outside application typechecking | [tsconfig.scripts.json](../tsconfig.scripts.json) checks TypeScript and MJS; [operational cases](../scripts/operational.test.mjs) cover script behavior | `pnpm typecheck:scripts` and `pnpm test:operations`. |
...
Local verification on Node 26.4.0 passed Biome, application/script/browser TypeScript checks, 289 Vitest cases, 10 Node operational cases, all four migration artifacts, a fixture production build, and 169 desktop/mobile browser checks. The browser runner exited normally in 45.6 seconds when allowed to clean up its own child process. The final production dependency audit reported no known vulnerabilities. CI separately exercises Node 22 and disposable PostgreSQL.
