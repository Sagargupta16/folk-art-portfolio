...
export async function readMigrations(directory = "drizzle") {
	...
	const snapshotFiles = (await readdir(resolve(root, "meta"))).filter((name) =>
		/^\d+_snapshot\.json$/.test(name),
	);
	matchArtifacts(
		sqlFiles,
	...
	for (const [index, entry] of journal.entries.entries()) {
		...
		requireValid(entry.idx === index, `Journal index ${index} is missing or out of order.`);
		requireValid(
		...
		const snapshot = JSON.parse(
			await readFile(resolve(root, "meta", `${prefix}_snapshot.json`), "utf8"),
		);
		requireValid(
