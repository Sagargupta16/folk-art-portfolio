...
function useParallax(frameRef: React.RefObject<HTMLDivElement | null>) {
  useEffect(() => {
    ...
    if (!frame) return;
    const shadow = frame.querySelector<HTMLDivElement>('[data-shadow]');
    const inner = frame.querySelector<HTMLDivElement>('[data-inner]');
...
export default function Hero() {
  ...
  return (
    <section className="relative overflow-hidden border-b border-[var(--color-line)]">
      ...
      <OrbitRing className="hidden md:block" style={{ bottom: '15%', left: '5%', position: 'absolute', width: '200px', height: '200px' }} />
      ...
      <div className="container-x relative z-10 grid gap-10 py-16 sm:gap-12 sm:py-24 md:grid-cols-12 md:items-center md:gap-10 md:py-32">
        <div className="stagger md:col-span-7">
          ...
          <ul className="reveal mt-7 flex flex-wrap gap-x-1.5 gap-y-1.5 sm:mt-10 sm:gap-x-3">
            {styles.map((s) => (
              <li key={s} className="t-meta rounded-full border border-[var(--color-line)] px-2.5 py-1 text-[0.65rem] sm:px-3 sm:text-xs">
                {s}
              </li>
            ))}
          </ul>
        ...
        <div className="md:col-span-5">
          <div className="mx-auto w-full max-w-md md:max-w-none" style={heroHaloStyle}>
            <div ref={frameRef} className="parallax-frame hero-halo relative aspect-[3/4] w-full">
              ...
              />
              <div data-inner="" className="relative h-full w-full overflow-hidden border border-[var(--color-line)]">
                {featured?.image ? (
                  <ArtworkImage
            ...
            {featured?.palette && (
              <div className="reveal mt-10 flex items-center gap-3">
                <Chromacard palette={featured.palette} ariaLabel={`Palette sampled from ${featured.title}`} />
