...
export default function Work() {
  ...
  return (
    <Section id="work" eyebrow={w.eyebrow} title={w.title} lead={w.lead}>
      <div>
        <div role="group" aria-label="Filter by style" className="mb-10 flex flex-wrap justify-center gap-2">
          {filters.map((f) => {
...
function GalleryCard({ item, onOpen }: { item: GalleryItem; onOpen: (slug: string) => void }) {
  const { ref, onMouseMove, onMouseLeave } = useTilt3D({ maxAngle: 6, scale: 1.03 });
  ...
  return (
    ...
    >
      <article className="group flex flex-col gap-4">
        ...
        >
          <ImageReveal direction={['left', 'right', 'up'][Math.abs(item.order) % 3] as 'left' | 'right' | 'up'}>
            {item.filename ? (
              <ArtworkImage
