...
const STYLE_PALETTE: Record<ArtStyle, { bg: string; ink: string; accent: string; soft: string }> = {
  Madhubani: { bg: '#f3d2a3', ink: '#7a1f12', accent: '#c0392b', soft: '#e8a87c' },
  Pichwai: { bg: '#dceee0', ink: '#1f4d3a', accent: '#2d6a4f', soft: '#88c4a3' },
  Lippan: { bg: '#efe1c8', ink: '#5a3a1d', accent: '#b08968', soft: '#d6b893' },
  Gond: { bg: '#dde4f0', ink: '#0f1d3a', accent: '#283c63', soft: '#7791b8' },
  Texture: { bg: '#ecdcd6', ink: '#3a1f1d', accent: '#7d4f50', soft: '#b89490' },
  'Mixed Media': { bg: '#e8dcc6', ink: '#3a2818', accent: '#8e735b', soft: '#bea98a' },
};
...
function concentricArches(p: { ink: string; accent: string; soft: string }, w: number, h: number): string {
  const cx = w / 2;
  const cy = h * 0.65;
  const out: string[] = [];
  for (let i = 1; i <= 7; i++) {
    ...
    out.push(`<path d="M ${cx - r} ${cy} A ${r} ${r} 0 0 1 ${cx + r} ${cy}" fill="none" stroke="${fill}" stroke-width="3" opacity="${0.85 - i * 0.08}"/>`);
  }
  out.push(`<circle cx="${cx}" cy="${cy}" r="${Math.min(w, h) / 14}" fill="${p.ink}" opacity="0.85"/>`);
  return out.join('\n  ');
}

function scallopGrid(p: { ink: string; accent: string; soft: string }, w: number, h: number, seed: number): string {
  const r = 36;
  const out: string[] = [];
  let s = seed;
  for (let y = r; y < h; y += r * 1.4) {
    for (let x = r; x < w; x += r * 1.4) {
      ...
      const stroke = p.ink;
      out.push(`<path d="M ${x - r} ${y} A ${r} ${r} 0 0 1 ${x + r} ${y}" fill="${fill}" stroke="${stroke}" stroke-opacity="0.35" stroke-width="1.5"/>`);
    }
  }
  return out.join('\n  ');
}
...
function petalRing(p: { ink: string; accent: string; soft: string }, w: number, h: number): string {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    const fill = i % 2 === 0 ? p.accent : p.soft;
    out.push(`<ellipse cx="${x}" cy="${y}" rx="${petalR * 0.55}" ry="${petalR * 0.28}" fill="${fill}" opacity="0.78" transform="rotate(${(angle * 180) / Math.PI} ${x} ${y})"/>`);
  }
...
function wovenStripes(p: { ink: string; accent: string; soft: string }, w: number, h: number): string {
  const out: string[] = [];
  const step = 28;
  for (let y = -h; y < h * 2; y += step) {
    out.push(`<line x1="0" y1="${y}" x2="${w}" y2="${y + w * 0.4}" stroke="${p.accent}" stroke-width="1.5" opacity="0.6"/>`);
  }
  for (let y = -h; y < h * 2; y += step) {
    out.push(`<line x1="0" y1="${y + step / 2}" x2="${w}" y2="${y + step / 2 - w * 0.4}" stroke="${p.soft}" stroke-width="1.5" opacity="0.5"/>`);
  }
  out.push(`<rect x="${w * 0.18}" y="${h * 0.32}" width="${w * 0.64}" height="${h * 0.36}" fill="${p.ink}" opacity="0.08"/>`);
  return out.join('\n  ');
}

function tessellatedDots(p: { ink: string; accent: string; soft: string }, w: number, h: number, seed: number): string {
  const out: string[] = [];
  let s = seed;
  const step = 32;
  ...
  out.push(`<circle cx="${w / 2}" cy="${h / 2}" r="${Math.min(w, h) / 5}" fill="${p.ink}" opacity="0.07"/>`);
  return out.join('\n  ');
}
